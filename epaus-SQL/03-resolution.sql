...
AS $$
BEGIN
    -- 1. Verifica se o NIF é nulo
    IF NEW.nif IS NULL THEN
        RAISE EXCEPTION 'NIF não pode ser NULL';
END IF;

    -- 2. Valida se contém 9 dígitos numéricos exatos
...
AS $$
BEGIN
    -- 1. Verificar duplicado para a tabela contacto_email
    IF TG_TABLE_NAME = 'contacto_email' THEN
        IF (TG_OP = 'INSERT') OR (TG_OP = 'UPDATE' AND NEW.email IS DISTINCT FROM OLD.email) THEN
            IF EXISTS (
                ...
                WHERE cliente_nif = NEW.cliente_nif AND email = NEW.email
            ) THEN
...
END IF;
    ...
    -- 2. Verificar duplicado para a tabela contacto_telefone
    IF TG_TABLE_NAME = 'contacto_telefone' THEN
        IF (TG_OP = 'INSERT') OR (TG_OP = 'UPDATE' AND NEW.telefone IS DISTINCT FROM OLD.telefone) THEN
            IF EXISTS (
                ...
                WHERE cliente_nif = NEW.cliente_nif AND telefone = NEW.telefone
            ) THEN
...
media NUMERIC;
BEGIN
    -- 1. Calcula a média móvel dos últimos N dias a partir dos registos diários do instrumento
...
RETURNS TABLE(isin VARCHAR(12), quantidade NUMERIC, valor_actual NUMERIC, percentagem_variacao_diaria NUMERIC)
LANGUAGE plpgsql
AS $$
BEGIN
    -- 1. Devolve uma listagem com os dados requeridos, cruzando posições com dados fundamentais
RETURN QUERY
SELECT
...
WHERE
    pos.portefolio = p_portefolio_id;
END;
$$;
-- endregion
...
instrumento_rec RECORD;
    ...
    open_val NUMERIC;
BEGIN
    -- 1. Loop por cada instrumento que teve triplos hoje
FOR instrumento_rec IN
SELECT DISTINCT identificador FROM triplo_externo WHERE data_tempo::date = CURRENT_DATE
    LOOP

-- 2. Descobrir o valor min e max de hoje
SELECT MIN(valor), MAX(valor)
INTO min_val, max_val
FROM triplo_externo
WHERE identificador = instrumento_rec.identificador AND data_tempo::date = CURRENT_DATE;

-- 3. Descobrir o valor de fecho (o último triplo de hoje, usando DESC)
SELECT valor INTO close_val
FROM triplo_externo
WHERE identificador = instrumento_rec.identificador AND data_tempo::date = CURRENT_DATE
ORDER BY data_tempo DESC
    LIMIT 1;

-- 4. Descobrir o valor de abertura (tentar ir buscar o fecho de ontem)
SELECT valor_fecho INTO open_val
FROM valor_instrumento_diario
WHERE instrumento_isin = instrumento_rec.identificador AND data < CURRENT_DATE
ORDER BY data DESC
    LIMIT 1;

-- 5. Se não houver dados de ontem (open_val continua NULL), a abertura é o primeiro triplo de hoje
IF open_val IS NULL THEN
SELECT valor INTO open_val
FROM triplo_externo
WHERE identificador = instrumento_rec.identificador AND data_tempo::date = CURRENT_DATE
...
END IF;
        ...
        -- 6. Guardar tudo na tabela (Inserir ou Atualizar se já existir conflito em data)
...
VALUES (instrumento_rec.identificador, CURRENT_DATE, min_val, max_val, open_val, close_val)
    ON CONFLICT (instrumento_isin, data) DO UPDATE SET
...
UPDATE cliente
SET nome = NEW.nome, cartao_cidadao = NEW.cartao_cidadao
WHERE nif = OLD.nif;
...
ALTER TABLE cliente ADD COLUMN IF NOT EXISTS version INTEGER NOT NULL DEFAULT 0;
...
BEGIN
    -- 1. Obter o mercado associado ao instrumento modificado
SELECT mercado INTO v_mercado FROM instrumento WHERE instrumento_id = NEW.instrumento_isin;

-- 2. Calcular a nova soma dos valores de abertura de todos os instrumentos desse mercado para o dia em causa
SELECT SUM(valor_abertura) INTO v_sum_abertura
FROM valor_instrumento_diario vid
         JOIN instrumento i ON i.instrumento_id = vid.instrumento_isin
WHERE i.mercado = v_mercado AND vid.data = NEW.data;

-- 3. Obter o valor de índice do mercado do dia anterior
v_ontem := NEW.data - INTERVAL '1 day';
SELECT valor_indice INTO v_valor_abertura_mercado
FROM valor_mercado_diario
WHERE mercado = v_mercado AND data = v_ontem;

IF v_valor_abertura_mercado IS NULL THEN
        v_valor_abertura_mercado := 0;
END IF;

    -- 4. Atualiza o valor diário do mercado (mantendo a coerência descrita nos requisitos)
INSERT INTO valor_mercado_diario(mercado, data, valor_indice, valor_abertura, variacao_diaria)
VALUES (v_mercado, NEW.data, v_sum_abertura, v_valor_abertura_mercado, v_sum_abertura - v_valor_abertura_mercado)
...
AFTER INSERT OR UPDATE ON valor_instrumento_diario
                           FOR EACH ROW
